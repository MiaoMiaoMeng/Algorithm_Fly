...
stereo26.c fichier "parametres" complété et renommé "setup", typiquement:
...
DISPLAY RATE    .3
COLOURS          1
            La selection a été complètement remaniée: subdivision de la population en
sous-populations et ranking rigoureux. Ca a obligé à rajouter des fonctions C.
            Corrigé un tres vilain bug sur stereobase qui etait lu dans setup meme quand
...
calculé sur toute l'image dès le début mais seulement sur les pixels dont on
a besoin, au fur et à mesure. On peut le visualiser grâce à la fonction
...
Ecriture de mutation0 (mutation sans père), elle se résume à un tirage
...
La fonction mutation0 n'est pas d'un très grand apport.
 ...
 le meilleur jeu de paramètres que l'on a trouvé pour 'money' est :
...
ZOOM                   2
DISPLAY RATE          .3
COLOURS                3  // penser à mettre à 3 quand on exploite une image en couleurs
TOURNAMENT             0
WINDOW SIZE (0 SMALL 1 BIG)      1
...
Le fichier setup est modifié en conséquence, avec le nouveau paramètre REMOTUM qui est
la distance max qui nous intéresse (et n'intervient que pour déterminer l'echelle et le cadrage
de l'affichage). Le parametre PROXIMUM sert à la fois à l'affichage et à l'algo lui-même.
...
/* Réécriture du gradient dans une fonction à part et utilisée dans eval() */
 ...
 for(n=0;n<POPULATION;n++)
   {
    if (indiv[n].life == 0)
      ...
      if ((xi+6<=width)&&(xi>=5)&&(eta+3<=height)&&(eta>=2)
        ...
        {
	  /* Calcul du gradient seulement où on en a besoin */
